...
/**
 * Writes packets to a XMPP server. Packets are sent using a dedicated(¡Á¡§¨®?¦Ì?) thread. Packet
 * interceptors can be registered to dynamically(?¡¥¨¬?¦Ì?) modify packets before they're actually
...
class PacketWriter {
    ...
    */ 
     ...
     * so we need to encapsulate(?1??) the logic in one place. This message will be sent while doing
