...
public class ConnectionConfiguration implements Cloneable {
    ...
    // Holds the socket factory that is used to generate【形成】 the socket in the connection
    ...
    protected void init(String serviceName, ProxyInfo proxy) {
        ...
        // Build the default[默认] path to the cacert[指定] truststore[受信认的] file. By default we are
        // going to use the file located in $JREHOME/lib/security/cacerts.
        String javaHome = System.getProperty("java.home");//jre的目录
        StringBuilder buffer = new StringBuilder();
        buffer.append(javaHome).append(File.separator).append("lib");//File.separator输出\
        buffer.append(File.separator).append("security");
        buffer.append(File.separator).append("cacerts");
        //得到的buffer="\lib\security\cacerts" 得到的是安全证书的路径
        
        truststorePath = buffer.toString();
        // Set the default store【存储】 type
        truststoreType = "jks";
