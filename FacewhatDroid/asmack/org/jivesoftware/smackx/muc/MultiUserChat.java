 ...
 * room. A room could have many occupants(?車℅???) with different affiliation(∩車那?) and roles.
 * Possible affiliatons are "owner", "admin", "member", and "outcast(谷⊿?赤)". Possible roles
 * are "moderator(?¯3?豕?)", "participant(2??車??)", and "visitor". Each role and affiliation 
 * guarantees different privileges (e.g. Send messages to all occupants, Kick participants 
 * and visitors,Grant voice, Edit member list, etc.).
 *
...
public class MultiUserChat {
    ...
    static {
        Connection.addConnectionCreationListener(new ConnectionCreationListener() {
            public void connectionCreated(final Connection connection) {
                // Set on every established(?“芍⊿) connection that this client supports the Multi-User
     ...
     *         don't have enough privileges to get this information.
     */
    public Collection<Affiliate> getAffiliatesByOwner(String affiliation) throws XMPPException {
        MUCOwner iq = new MUCOwner();
        iq.setTo(room);
        iq.setType(IQ.Type.GET);
